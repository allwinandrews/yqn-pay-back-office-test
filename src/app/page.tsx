...
export default function Home() {
  ...
  async function loadTransactions() {
    ...
    if (appliedFilters.merchantId)
      params.set("merchantId", appliedFilters.merchantId);
    if (appliedFilters.reference) params.set("reference", appliedFilters.reference);
    if (appliedFilters.from) params.set("from", appliedFilters.from);
    if (appliedFilters.to) params.set("to", appliedFilters.to);
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-6 py-10">
        <header className="flex flex-col gap-2">
          ...
          <p className="max-w-2xl text-sm text-slate-500">
            Record merchant payments, reverse errors without deleting history,
            and export clean CSVs for finance.
          </p>
        </header>
